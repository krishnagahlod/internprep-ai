...
def extract_user_bullets(resume_text: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        res = gemini_client.generate_content(os.getenv("PREPROCESSING_MODEL", "gemini-flash-latest"), prompt, generation_config=config)
...
def analyze_resume_text(resume_text: str, target_role: str = "consulting", pdf_bytes: bytes = None) -> str:
    ...
    response = gemini_client.generate_content(os.getenv("ANALYSIS_MODEL", "gemini-flash-latest"), final_prompt, generation_config=config)
